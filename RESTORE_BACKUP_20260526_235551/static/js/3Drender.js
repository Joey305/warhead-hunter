...
async function headOrGetOk(url){
  // Try HEAD first (lightweight). If server blocks HEAD, fall back to GET.
  try{
    let r = await fetch(url, { method: "HEAD", cache: "no-store" });
    if (r.status === 405) { // method not allowed
...
async function validateAndRevealCards(){
  const jobId = (window.PROTACABLE_CONFIG && window.PROTACABLE_CONFIG.job_id) || null;
  if (!jobId) return;

  const cards = Array.from(document.querySelectorAll(".result-card"));
  if (!cards.length) return;

  // If you want to require SASA atoms to exist, keep REQUIRE_SASA=true.
  // If you only want to require 3D (SDF) and allow missing SASA, set false.
  const REQUIRE_SASA = true;

  // Validate sequentially (less bursty on your server). If you want faster, use Promise.allSettled.
  for (const card of cards){
    const pdb   = (card.dataset.pdb || "").trim();
    const chain = (card.dataset.chain || "").trim();
    const war   = (card.dataset.warhead || "").trim();
    ...
    // 3D exists?
    const sdfUrl = `/api/sdf/${encodeURIComponent(jobId)}/${encodeURIComponent(pdb)}/${encodeURIComponent(chain)}/${encodeURIComponent(war)}`;
    ...
    // SASA atoms exists? (this is the one returning 404 in your logs)
    let okSasa = true;
    if (REQUIRE_SASA){
      if (!resid){
        okSasa = false;
        ...
        const sasaUrl =
          `/api/jobs/${encodeURIComponent(jobId)}/sasa/atoms?` +
          `pdb_id=${encodeURIComponent(pdb)}&chain=${encodeURIComponent(chain)}&residue_id=${encodeURIComponent(resid)}`;
        okSasa = await headOrGetOk(sasaUrl);
      }
    }
    ...
    // Passed validation -> reveal
    card.classList.remove("pending");
  }

  // Optional: if the currently “selected” card was removed, pick the first remaining card
  const first = document.querySelector(".result-card:not(.pending)");
  if (first && typeof first.click === "function"){
    // Only auto-click if nothing selected yet; adjust if you track selection state.
    // first.click();
  }
}

// Run after DOM is ready
document.addEventListener("DOMContentLoaded", validateAndRevealCards);
