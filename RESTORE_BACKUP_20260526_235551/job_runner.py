...
# # Structure: { job_id: { "status": "running", "log": [], "output_path": "" } }
# JOB_STORE = {}
...
# def run_script_logged(job_id, script_name, args, job_dir, timeout_sec=None):
...
#     # -u also forces unbuffered output from python scripts
#     cmd = ["python3", "-u", script_name] + args

#     start = time.time()
#     JOB_STORE[job_id]["current_step"] = script_name
#     JOB_STORE[job_id]["step_started_at"] = datetime.now().strftime("%H:%M:%S")
...
#                 proc.kill()
#                 raise Exception(f"{script_name} timed out after {timeout_sec}s")

#         proc.wait()

#     if proc.returncode != 0:
#         raise Exception(f"{script_name} failed with code {proc.returncode}")
...
#         JOB_STORE[job_id]["log"].append(entry)

# def run_pipeline_task(job_id, target_name, search_query, fasta_seq):
...
#         # 1. Copy all assets to the job folder (So we don't pollute the main folder)
...
#         # 2. Create the Input CSV (Protein_Data.csv) required by Script 1 & 2
#         # Note: 1_GRABBER needs 'protein' and 'search_query'
#         input_data = [{
#             "protein": target_name, 
#             "search_query": search_query,
#             "fasta": fasta_seq  # Used by SQchk
#         }]
#         pd.DataFrame(input_data).to_csv(os.path.join(job_dir, "input.csv"), index=False)
#         pd.DataFrame(input_data).to_csv(os.path.join(job_dir, "Protein_Data.csv"), index=False) # Helper for later scripts

#         # 3. Define the Execution Sequence
#         # We use subprocess to run them exactly as if we were in the CLI
#         # Ensure every line is ( "filename.py", [list, of, args] )
#         scripts = [
...
#             ("11_mcsMatcher.py", [job_id]),
#             ("12_Results.py", []),
#             # ("13_mcsSASA_svg.py", []),
#             # Script 13 removed to prevent data corruption
           
#             ("15_ResultsMerged.py", []),
#             ("16_ResultsDisplay.py", [job_id]),
#             #  ("14_SVGmkr.py", [job_id]),
#         ]
...
#             # Run command inside the job_dir so files generate there
#             cmd = ["python3", script_name] + args
            
#             # Capture output
#             with subprocess.Popen(cmd, cwd=job_dir, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True) as proc:
#                 for line in proc.stdout:
#                     JOB_STORE[job_id]["log"].append(line.strip())
#                 for line in proc.stderr:
#                     JOB_STORE[job_id]["log"].append(f"ERR: {line.strip()}")
...
# def start_job(target_name, search_query, fasta_seq):
...
#     thread = threading.Thread(target=run_pipeline_task, args=(job_id, target_name, search_query, fasta_seq))
#     thread.daemon = True
#     thread.start()
