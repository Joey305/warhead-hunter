...
def download_single_cif(pdb_id, outdir):
    ...
    if outfile.exists(): return pdb_id, True, "cached"

    # List of sources to try
    # 1. RCSB Files (Primary)
    # 2. EBI Files (Backup)
    sources = [
    ...
    if ids:
        with ThreadPoolExecutor(max_workers=MAX_DOWNLOAD_WORKERS) as exe:
            ...
            for fut in as_completed(futures):
                ...
                if ok: downloaded.append(pdb_id)
                else: print(f"    ✖ {pdb_id} ({msg})")
...
def main():
    ...
    parser.add_argument("--outdir", type=str, default="WAR")
    parser.add_argument("--limit", type=int, default=None)
    parser.add_argument("--mw_min", type=float, default=250.0)
    parser.add_argument("--mw_max", type=float, default=None)
    ...
    if len(sys.argv) > 1:
        ...
        args = argparse.Namespace(auto=str(csvs[0]), outdir="WAR", limit=None, mw_min=250.0, mw_max=None, min_id=90.0)
    ...
    pd.DataFrame([{"MW_min": args.mw_min, "MW_max": args.mw_max, "MinID": args.min_id}]).to_csv("queries.csv", index=False)
