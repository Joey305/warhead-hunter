...
def write_sdf_obabel(mol: Chem.Mol, out_sdf: Path, name: str = "") -> Tuple[bool, str]:
    """
    Write a temp PDB from RDKit (preserves atom order), then convert via obabel.
    """
    try:
        out_sdf.parent.mkdir(parents=True, exist_ok=True)

        # unique temp name per instance output (safe under multiprocessing)
        ...
        try:
            ...
            try:
                ...
                pass
            return False, "obabel timeout (>10s)"
        ...
        return False, f"obabel exception: {e}"
...
def compute_mcs(key, grouped):
    ...
    # resolve rewritten ligands (A00 -> A1A, etc) for SMILES
    alias_key = (target, str(pdb_id).lower(), ligand)

    candidates = []

    # 1️⃣ Prefer ligand5 ALWAYS
    ...
    # 3️⃣ Finally fallback to original ligandX
    ...
    # Persisted SMILES columns (THIS is the key to making SVGs consistent)
    ...
    try:
        ...
        # --- Optional: write 3D SDF per ligand occurrence ---
        if WRITE_SDF:
            ...
            if (not SKIP_IF_EXISTS) or (not out_sdf.exists()):
                ...
                if not ok_sdf:
                    ...
                    print(f"🧪 SDF {base} → {out_sdf.name} ({dt:.2f}s) {msg_sdf}")
    ...
    # if no 2D, output -1 for all atoms (but STILL include SMILES columns)
    if mol2d is None:
        rows_all = []
    ...
    # PASS A0: bond-generic full match (fast)
    try:
    ...
    # PASS A: AssignBondOrdersFromTemplate (only if counts match)
    ...
    # PASS B: MCS on generic graphs (robust for size mismatch)
    if not map_3d_to_2d:
        try:
            q2 = make_bonds_generic(mol2d)
            q3 = make_bonds_generic(mol3d)
...
if __name__ == "__main__":
    ...
    with Pool(processes=NPROC, maxtasksperchild=50) as pool:
        ...
        for rows_mcs, rows_all, errs in pool.imap_unordered(compute_mcs_timed, keys, chunksize=1):
            if rows_mcs:
                results_mcs.extend(rows_mcs)
            if rows_all:
                results_all.extend(rows_all)
            if errs:
