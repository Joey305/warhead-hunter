...
The results gallery sends selected warhead SMILES to PROTAC Builder using `PROTAC_BUILDER_BASE`. By default this is:

```text
https://protacbuilder.com/copy/COPYindex
```

Local/private development deployments can override it with the `PROTAC_BUILDER_BASE` environment variable, for example a private Tailscale URL. No source file requires that private URL to function.
