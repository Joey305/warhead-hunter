(() => {
  ...
  async function headOrGetStatus(url) {
    try {
      let r = await fetch(url, { method: "HEAD", cache: "no-store" });
  ...
  function incrementRemoved(reason) {
    State.debug.removed[reason] = (State.debug.removed[reason] || 0) + 1;
  }

  function ensureDebugPanel() {
    if (!DEBUG_MODE) return null;

    let panel = $("ligand-debug-panel");
    if (panel) return panel;

    panel = document.createElement("div");
    panel.id = "ligand-debug-panel";
    panel.style.cssText = [
      "position:fixed",
      "right:12px",
      "bottom:12px",
      ...
      "box-shadow:0 0 18px rgba(255,214,0,.18)"
    ].join(";");
  ...
  function updateDebugPanel() {
    ...
    const removedRows = Object.entries(State.debug.removed)
      .map(([reason, count]) => `<div>removed ${escapeHtml(reason)}: ${count}</div>`)
      .join("") || "<div>removed: 0</div>";

    panel.innerHTML = `
      <div style="font-weight:700;margin-bottom:4px;">Ligand display debug</div>
      <div>total result cards: ${State.debug.total}</div>
      <div>protein OK: ${State.debug.proteinOk}</div>
      <div>SDF OK: ${State.debug.sdfOk}</div>
      <div>SASA OK: ${State.debug.sasaOk}</div>
      <div>kept missing SASA: ${State.debug.keptMissingSasa}</div>
      ${removedRows}
    `;
  }
  ...
  function getCardIdentifiers(card) {
    return {
      pdb: String(card.dataset.pdb || "").trim().toLowerCase(),
      chain: String(card.dataset.chain || "").trim().toUpperCase(),
      warhead: String(card.dataset.warhead || "").trim().toUpperCase(),
      resid: String(card.dataset.resid || "").trim()
    };
  }

  async function validateCardAssets(card, { checkSasa = false } = {}) {
    const ids = getCardIdentifiers(card);

    if (!ids.pdb || !ids.chain || !ids.warhead) {
      return { ...ids, renderable: false, reason: "missing identifiers" };
    }

    const proteinUrl =
      `/api/protein/${encodeURIComponent(JOB_ID)}/${encodeURIComponent(ids.pdb)}/${encodeURIComponent(ids.chain)}`;

    const sdfUrl =
      `/api/sdf/${encodeURIComponent(JOB_ID)}/${encodeURIComponent(ids.pdb)}/${encodeURIComponent(ids.chain)}/${encodeURIComponent(ids.warhead)}`;

    const protein = await headOrGetStatus(proteinUrl);
    const sdf = await headOrGetStatus(sdfUrl);

    if (!protein.ok) {
      return { ...ids, protein, sdf, renderable: false, reason: `protein ${protein.status || "network"}` };
    }
    if (!sdf.ok) {
      return { ...ids, protein, sdf, renderable: false, reason: `sdf ${sdf.status || "network"}` };
    }

    if (!ids.resid) {
      ids.resid = await resolveResid(ids.pdb, ids.chain, ids.warhead, "");
      if (ids.resid) card.dataset.resid = ids.resid;
    }

    let sasa = { ok: false, status: 0, skipped: true };
    if (checkSasa && ids.resid) {
      const sasaUrl =
        `/api/jobs/${encodeURIComponent(JOB_ID)}/sasa/atoms?` +
        `pdb_id=${encodeURIComponent(ids.pdb)}` +
        `&chain=${encodeURIComponent(ids.chain)}` +
        `&residue_id=${encodeURIComponent(ids.resid)}`;

      sasa = await headOrGetStatus(sasaUrl);
    }

    return { ...ids, protein, sdf, sasa, renderable: true, reason: "" };
  }

  async function findFirstRenderableCard(cards) {
    for (const card of cards) {
      const result = await validateCardAssets(card, { checkSasa: false });
      if (result.renderable) {
        card.classList.remove("pending");
        return { card, result };
      }
    }
    return { card: null, result: null };
  }

  async function filterRenderableCards() {
    const cards = Array.from(document.querySelectorAll(".result-card"));
    State.debug.total = cards.length;
    State.debug.proteinOk = 0;
    State.debug.sdfOk = 0;
    State.debug.sasaOk = 0;
    State.debug.keptMissingSasa = 0;
    State.debug.removed = {};
    updateDebugPanel();

    for (const card of cards) {
      const result = await validateCardAssets(card, { checkSasa: true });

      if (!result.renderable) {
        incrementRemoved(result.reason || "not renderable");
        card.remove();
        updateDebugPanel();
        continue;
      }

      State.debug.proteinOk += 1;
      State.debug.sdfOk += 1;

      if (result.sasa && result.sasa.ok) {
        State.debug.sasaOk += 1;
        card.classList.remove("sasa-missing");
        card.dataset.sasaAvailable = "true";
      } else {
        State.debug.keptMissingSasa += 1;
        card.classList.add("sasa-missing");
        card.dataset.sasaAvailable = "false";
        console.warn(
          "[LigandDisplay] SASA atoms unavailable; keeping ligand card and disabling overlay.",
          {
            key: `${result.pdb}|${result.chain}|${result.resid || ""}`,
            status: result.sasa?.status,
            url: result.sasa?.url,
            error: result.sasa?.error
          }
        );
      }
...
function openBuilderFallback(smiles, opts = {}) {
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    const { card: first } = await findFirstRenderableCard(cards);
      ...
      if (viewport) {
        viewport.innerHTML = `
          <div style="padding:24px;color:#ffd600;font-family:monospace;">
            No complete ligand/protein/SDF entries found for this job.
          </div>
        `;
      }
    }

    filterRenderableCards().catch(err => {
      console.warn("[LigandDisplay] Optional card diagnostics failed:", err);
    });
  });
})();
