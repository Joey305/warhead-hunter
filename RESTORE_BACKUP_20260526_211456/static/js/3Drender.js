(() => {
  ...
  async function loadProtein(pdb, chain) {
    ...
    console.log("[Render3D] protein URL:", url);
  ...
  async function loadLigand(pdb, chain, warhead) {
    ...
    console.log("[Render3D] SDF URL:", url);
  ...
  async function fetchSasaAtoms(pdb, chain, resid) {
    ...
    console.log("[Render3D] SASA URL:", url);
  ...
  async function load({ pdb, chain, warhead, resid }) {
    ...
    let ligandLoaded = false;

    try {
      try {
        State.proteinComp = await loadProtein(PDB, CH);
      } catch (e) {
        console.warn("[Render3D] Protein load failed", {
          pdb: PDB,
          chain: CH,
      ...
      try {
        ...
        ligandLoaded = true;
        setHudDebug("Ligand loaded.");
      } catch (e) {
        console.warn("[Render3D] Ligand load failed", {
          pdb: PDB,
          chain: CH,
          warhead: WAR,
          message: e?.message || String(e),
      ...
      if (RES) {
        try {
          ...
          console.warn("[Render3D] SASA overlay unavailable; ligand remains visible.", {
            key: `${PDB}|${CH}|${RES}`,
            message: e?.message || String(e),
            status: e?.status,
            url: e?.url,
          ...
          if (ligandLoaded) {
            setHudDebug(`Ligand loaded. SASA overlay unavailable for ${PDB}|${CH}|${RES}.`);
          } else {
            setHudDebug(`SASA overlay failed: ${e.message || e}`);
          }
        }
      } else if (ligandLoaded) {
        setHudDebug("Ligand loaded. SASA overlay skipped: missing resid.");
      } else {
