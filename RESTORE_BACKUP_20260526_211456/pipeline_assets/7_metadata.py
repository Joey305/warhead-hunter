...
def main():
    ...
    if df.empty:
        print("ℹ️ No ligand rows remain after filtering; writing empty downstream outputs and exiting cleanly.")

        pd.DataFrame(columns=["Rank", "Ligand_Resolved", "PDB_List", "Count"]).to_csv(
            "Ligand_PDB_Index.csv", index=False
        )
        ...
        pd.DataFrame(columns=metadata_header).to_csv("Ligand_Metadata.csv", index=False)
        pd.DataFrame(columns=metadata_header + ["Error"]).to_csv("Ligand_Metadata_Failures.csv", index=False)

        print("💾 Saved empty Ligand_PDB_Index.csv")
        print("💾 Saved empty Ligand_Metadata.csv")
        print("💾 Saved empty Ligand_Metadata_Failures.csv")
        print("\n🎉 COMPLETE → No resolved ligands to annotate")
        print("===================================================================")
        return
