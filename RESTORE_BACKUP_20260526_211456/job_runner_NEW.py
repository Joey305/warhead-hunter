...
def _describe_artifact(path: str) -> str:
    if os.path.isdir(path):
        return f"dir exists files={_count_tree_files(path)}"
    return _describe_path(path)

def _log_artifact_snapshot(job_id: str, job_dir: str, label: str) -> None:
    watched = [
        ("WAR_PDB", os.path.join(job_dir, "WAR_PDB")),
        ("TARGET_RESULTS", os.path.join(job_dir, "TARGET_RESULTS")),
        ("TARGET_RESULTS/LIGAND_PDB", os.path.join(job_dir, "TARGET_RESULTS", "LIGAND_PDB")),
        ("TARGET_RESULTS/LIGAND_SDF", os.path.join(job_dir, "TARGET_RESULTS", "LIGAND_SDF")),
        ("Warhead_SASA_summary.csv", os.path.join(job_dir, "Warhead_SASA_summary.csv")),
        ("Resolved_SASA_Summary.csv", os.path.join(job_dir, "Resolved_SASA_Summary.csv")),
        ("Results_Display.csv", os.path.join(job_dir, "Results_Display.csv")),
        ("5CharMAP.csv", os.path.join(job_dir, "5CharMAP.csv")),
        ("components-smiles-stereo-oe.smi", os.path.join(job_dir, "components-smiles-stereo-oe.smi")),
    ]
    for name, path in watched:
        log_message(job_id, f"[artifacts after {label}] {name}: {_describe_artifact(path)}")

def _resolve_asset_source(dest_name: str) -> Optional[str]:
    candidates = REQUIRED_PIPELINE_ASSETS.get(dest_name, [dest_name])
    for candidate in candidates:
        src = os.path.join(ASSET_DIR, candidate)
...
def _copy_assets(job_id: str, job_dir: str) -> None:
    ...
    missing_source_assets = [
        ...
        if _resolve_asset_source(name) is None
    ]
    ...
    # Canonicalize critical asset names inside each job workspace.
    for dest_name in REQUIRED_PIPELINE_ASSETS:
        src = _resolve_asset_source(dest_name)
        if src is None:
            continue
        dst = os.path.join(job_dir, dest_name)
        shutil.copy2(src, dst)
        log_message(job_id, f"📎 Verified asset {dest_name}: {_describe_path(dst)}")
...
def _write_inputs(job_id: str, job_dir: str, target_name: str, search_query: str, fasta_seq: str) -> None:
    ...
    _log_artifact_snapshot(job_id, job_dir, "input setup")
...
def run_pipeline_task(job_id: str, target_name: str, search_query: str, fasta_seq: str) -> None:
    ...
    try:
        ...
        scripts: List[Tuple[str, List[str]]] = [
            ...
            ("12_Results.py", []),
            ("14_SVGmkr.py", []),
            # Step 17 backfills any missing legacy ligand SDF files from LIGAND_PDB.
            ("17_obabelSDF.py", []),
            ("15_ResultsMerged.py", []),
        ...
        for script_name, args in scripts:
            try:
                ...
                _log_artifact_snapshot(job_id, job_dir, script_name)
        ...
        try:
            ...
            _log_artifact_snapshot(job_id, job_dir, CLEANUP_SCRIPT_NAME)
