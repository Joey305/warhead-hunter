...
def _read_job_metadata_summary(job_id: str) -> Dict[str, str]:
    job_path = _job_dir(job_id)
    fp = job_path / "job_metadata.json"
    if not fp.exists():
        return {}
    try:
...
def view_results(job_id: str):
    ...
    if df is None or df.empty:
        job_path = _job_dir(job_id)
        metadata = _read_job_metadata_summary(job_id)
        candidates = [
            job_path / "Resolved_SASA_Summary.tsv",
            job_path / "Resolved_SASA_Summary.csv",
            job_path / "TARGET_RESULTS" / "Resolved_SASA_Summary.tsv",
            job_path / "TARGET_RESULTS" / "Resolved_SASA_Summary.csv",
        ]
        summary_fp = _first_existing(candidates)
        if summary_fp and summary_fp.exists():
            try:
                size_part = f"{summary_fp.stat().st_size} bytes"
            except Exception:
                size_part = "size unknown"
            summary_state = f"{summary_fp.relative_to(job_path)} exists ({size_part})"
        else:
            summary_state = "Resolved_SASA_Summary.csv/tsv is missing"

        status = metadata.get("status") or "unknown"
        current_step = metadata.get("current_step") or "none"
        error_message = metadata.get("error_message") or "none recorded"
        abort(
            404,
            description=(
                f"Job {job_id} exists, but no displayable results were found. "
                f"Status={status}, current_step={current_step}, summary_state={summary_state}, "
                f"error={error_message}. Try /api/jobs/{job_id}, /api/jobs/{job_id}/results, "
                f"or /api/jobs/{job_id}/files for raw diagnostics."
            ),
        )
