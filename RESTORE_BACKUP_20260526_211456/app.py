...
def _sdf_404_details(job_id: str, pdb: str, chain: str, ligand: str) -> Dict[str, Any]:
    job_base = safe_job_dir(job_id)
    target_dir = target_results_dir(job_id)
    legacy_dir = ligand_sdf_dir(job_id)
    mcs_dir = mcs_sdf_dir(job_id)

    nearby: List[str] = []
    for folder in [mcs_dir, legacy_dir]:
        if folder and folder.exists():
            for fp in sorted(folder.glob("*.sdf"))[:10]:
                nearby.append(str(fp.relative_to(job_base)) if job_base and fp.is_relative_to(job_base) else str(fp))

    expected_paths = []
    if mcs_dir:
        expected_paths.append(str(mcs_dir / f"{pdb}_{chain}_{ligand}_<resid>.sdf"))
    else:
        expected_paths.append(f"(missing MCS dir) {job_id}/TARGET_RESULTS/MCS_Output/MCS_SDF/{pdb}_{chain}_{ligand}_<resid>.sdf")
    if legacy_dir:
        expected_paths.append(str(legacy_dir / f"{pdb}_{chain}_{ligand}.sdf"))
    else:
        expected_paths.append(f"(missing legacy dir) {job_id}/TARGET_RESULTS/LIGAND_SDF/{pdb}_{chain}_{ligand}.sdf")
    ...
    return {
        ...
        "job_dir_exists": bool(job_base and job_base.exists()),
        "target_results_exists": bool(target_dir and target_dir.exists()),
        "ligand_sdf_dir_exists": bool(legacy_dir and legacy_dir.exists()),
        "mcs_sdf_dir_exists": bool(mcs_dir and mcs_dir.exists()),
        "nearby_sdf_files": nearby,
    }
...
def api_sdf(job_id, pdb, ligand, chain=None):
    ...
    details = _sdf_404_details(job_id, pdb, chain, ligand)
    app.logger.warning("SDF lookup failed: %s", details)
...
def not_found(e):
    message = getattr(e, "description", None) or "Page not found."
